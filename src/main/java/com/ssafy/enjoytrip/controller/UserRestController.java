...
public class UserRestController {
	...
	public ResponseEntity<?> join(@RequestBody UserDto userDto){
	    ...
	    try {
	        ...
	        // [수정 포인트] 서비스에서 던진 "중복 에러"를 낚아채는 부분!
	        // 예: "이미 사용 중인 닉네임입니다." 메시지와 함께 409(Conflict) 상태 코드 반환
    ...
    public ResponseEntity<?> nicknameCheck(@PathVariable("nickname") String nickname) {
        ...
        try {
            ...
            return new ResponseEntity<Integer>(cnt, HttpStatus.OK); // 1이면 중복, 0이면 사용 가능
        } catch (Exception e) {
            e.printStackTrace();
            return exceptionHandling(e);
        }
    ...
    /**
     * 9. 전화번호 중복 체크 (GET)
     * URL: /user/phonecheck/01012345678
     */
    @GetMapping("/phonecheck/{phoneNumber}")
    public ResponseEntity<?> phoneNumberCheck(@PathVariable("phoneNumber") String phoneNumber) {
        log.debug("전화번호 중복 체크 요청: {}", phoneNumber);
        try {
            int cnt = userService.phoneNumberCheck(phoneNumber);
            return new ResponseEntity<Integer>(cnt, HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
            return exceptionHandling(e);
